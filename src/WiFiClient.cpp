...
int WiFiClient::connect(IPAddress ip, uint16_t port, int32_t timeout)
{
    ...
    if (res < 0 && errno != EINPROGRESS) {
        printf("connect on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
        shutdown(sockfd,SHUT_RDWR);
        return 0;
    }
    ...
    if (res < 0) {
        printf("select on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
        shutdown(sockfd,SHUT_RDWR);
        return 0;
    } else if (res == 0) {
        printf("select returned due to timeout %d ms for fd %d", timeout, sockfd);
        shutdown(sockfd,SHUT_RDWR);
        return 0;
    } else {
        int sockerr;
        ...
        if (res < 0) {
            printf("getsockopt on fd %d, errno: %d, \"%s\"", sockfd, errno, strerror(errno));
            shutdown(sockfd,SHUT_RDWR);
            return 0;
        }

        if (sockerr != 0) {
            printf("socket error on fd %d, errno: %d, \"%s\"", sockfd, sockerr, strerror(sockerr));
            shutdown(sockfd,SHUT_RDWR);
            return 0;
        }
    }
...
int WiFiClient::setOption(int option, int *value)
{
    ...
    if(res < 0) {
        printf("fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
    }
    return res;
}
...
uint8_t WiFiClient::connected()
{
    if (_connected) {
        ...
        switch (errno) {
            ...
            case ECONNABORTED:
                _connected = false;
                printf("Disconnected: RES: %d, ERR: %d", res, errno);
                break;
            default:
                printf("Unexpected: RES: %d, ERR: %d", res, errno);
                _connected = true;
                break;
        }
    }
